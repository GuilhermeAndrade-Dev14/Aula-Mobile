...
export default function App() {
 ...
 async function buscarPaisRandom() {
  try {
    ...
    const indiceRandom = Math.floor(
      Math.random() * paises.length
    );

    setPais(paises[indiceRandom]);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity style={styles.botao2} onPress={buscarPais}>
        <Text style={styles.textoBotao}>Buscar País</Text>
      </TouchableOpacity>

      <TouchableOpacity
        style={styles.botao2}
        onPress={buscarPaisRandom}
      >
